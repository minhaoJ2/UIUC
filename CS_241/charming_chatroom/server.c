...
void run_server(char *port) {
    ...
    int opt_v = 1;
    if(setsockopt(sock_fd, SOL_SOCKET, SO_REUSEADDR, &opt_v, sizeof(int))) {
    ...
    while(endSession == 0) {
        ...
        if(i == MAX_CLIENTS) {
            int tmp = shutdown(client_fd, SHUT_RDWR);
            if(tmp != 0) perror("failt to shutdown");
            close(client_fd);
            continue;
        }
        
        clients[i] = client_fd;
        pthread_create(tids+i, NULL, process_client, (void *)i);
